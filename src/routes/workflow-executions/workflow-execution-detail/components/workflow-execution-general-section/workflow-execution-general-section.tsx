...
export const WorkflowExecutionGeneralSection = ({
  ...
  const translatedState = getTransactionState(t, execution.state as TransactionState);
  const stateColor = getTransactionStateColor(execution.state as TransactionState);

  return (
    <Container
      className="divide-y p-0"
      data-testid="workflow-execution-general-section"
    >
      <div
        className="flex items-center justify-between px-6 py-4"
        data-testid="workflow-execution-general-section-header"
      >
        <div
          className="flex items-center gap-x-0.5"
          data-testid="workflow-execution-general-section-id"
        >
          <Heading data-testid="workflow-execution-general-section-heading">{cleanId}</Heading>
          <Copy
            content={cleanId}
            className="text-ui-fg-muted"
            data-testid="workflow-execution-general-section-copy"
          />
        </div>
        <StatusBadge
          color={stateColor}
          data-testid="workflow-execution-general-section-status-badge"
        >
          {translatedState}
        </StatusBadge>
      </div>
      <div
        ...
        data-testid="workflow-execution-general-section-workflow-id-row"
      >
        <Text
          size="small"
          leading="compact"
          weight="plus"
          data-testid="workflow-execution-general-section-workflow-id-label"
        >
        ...
        <Badge
          size="2xsmall"
          className="w-fit"
          data-testid="workflow-execution-general-section-workflow-id-value"
        >
      ...
      <div
        className="grid grid-cols-2 px-6 py-4 text-ui-fg-subtle"
        data-testid="workflow-execution-general-section-transaction-id-row"
      >
        <Text
          size="small"
          leading="compact"
          weight="plus"
          data-testid="workflow-execution-general-section-transaction-id-label"
        >
        ...
        <Badge
          size="2xsmall"
          className="w-fit"
          data-testid="workflow-execution-general-section-transaction-id-value"
        >
      ...
      <div
        className="grid grid-cols-2 px-6 py-4 text-ui-fg-subtle"
        data-testid="workflow-execution-general-section-progress-row"
      >
        <Text
          size="small"
          leading="compact"
          weight="plus"
          data-testid="workflow-execution-general-section-progress-label"
        >
          {t('workflowExecutions.progressLabel')}
        </Text>
        <Progress
          steps={execution.execution?.steps}
          data-testid="workflow-execution-general-section-progress-value"
        />
      </div>
    </Container>
  );
        ...
        {actionableSteps.map(step => (
          <div
            ...
            className={clx(
              ...
              {
                'bg-ui-fg-muted': step.invoke.state === TransactionStepState.DONE
              }
            )}
